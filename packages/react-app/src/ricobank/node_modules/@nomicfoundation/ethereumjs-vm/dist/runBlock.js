...
async function runBlock(opts) {
    ...
    if (this._hardforkByBlockNumber ||
        ...
        this._common.setHardforkByBlockNumber(block.header.number, opts.hardforkByTTD ?? this._hardforkByTTD, block.header.timestamp);
    }
    if (this.DEBUG) {
        debug('-'.repeat(100));
...
async function assignWithdrawals(block) {
    ...
    for (const withdrawal of withdrawals) {
        ...
        // converted to wei
        await rewardAccount(state, address, amount * ethereumjs_util_1.GWEI_TO_WEI);
    }
}
/**
